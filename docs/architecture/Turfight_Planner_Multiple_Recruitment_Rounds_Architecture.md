...
Rootは `activeRecruitmentRoundId` を、各Recruitment Roundは `activePlanId` を保持する。保存時は現在募集回と現在Planだけを既存Rootへマージし、読込時はRootから募集回、募集回からPlanの順に選択状態を解決する。現行schemaVersion 3はPlanを親としているため、移行時にPlan×募集回の状態を募集回ごとのPlanへ展開する。各Recruitment Roundは最低1件のPlanを持ち、旧Planから1件も生成できない場合は決定的なdefault Planを補完する。複数募集回UIより先に保存・適用責任を分離し、非アクティブな募集回とPlanを保持できる保存基盤を完成させる。
...
移行後の各Recruitment Roundは、次の不変条件を満たさなければならない。

- `plans` はArrayであり、最低1件のPlanを持つ。
- `activePlanId` は必ず同じRecruitment Roundの `plans[].id` に存在する。
- 旧PlanからPlanを生成できない場合も、同一入力に対して同一のdefault Planを1件だけ生成する。
- 不変条件を満たさないデータは書込み前検証で失敗させ、元の保存文字列を上書きしない。
...
6. そのroundに対応する旧PlanからPlanを1件も生成できない場合は、次のdefault Planを1件だけ生成する。
...
7. `eventBudget`、`voucherAmount`、`bulkRate` は募集回設定へ移す。
8. Rootの旧 `activePlanId` が各募集回内の `plans[].id` に存在すれば、その回の `activePlanId` とする。存在しなければ先頭PlanのIDへ解決する。
9. `plans` が空、または `activePlanId` が `plans[].id` に存在しない状態を、書込み前検証で失敗させる。
10. Rootの `activeRecruitmentRoundId` は旧active Planの `activeRecruitmentRoundId`、カタログ先頭回の順で解決する。
11. Rootの `schemaVersion` を4にする。
12. v4を再入力した場合は既存の `plans` とdefault Planをそのまま検証し、default Planを追加生成しない。これにより `migrateToV4()` の冪等性を維持する。
...
- `plans` が空: Recruitment Roundの不変条件違反として書込み前検証を失敗させる。
- `activePlanId` 不整合: 同じRecruitment Roundの `plans[].id` を参照しない場合は書込み前検証を失敗させる。
...
| R7 | Proposal | 保存責任の分離前に複数募集回UIを導入すると、全体置換により非アクティブな募集回や他Planを失い得る | Issue DをIssue B・Cより先に実施し、対象募集回・対象Planだけをupsertする基盤を完成させる |
| R8 | Proposal | round IDだけが生成され、対応する旧Planがない場合に `plans` が0件となり得る | default Planを決定的に1件生成し、`plans` 非空と `activePlanId` の参照整合性を検証する |
...
- v3の設定競合診断を実装
- Planを生成できない募集回へdefault Planを決定的に補完し、`plans` 非空と`activePlanId`の参照整合性を検証する
- 完了条件: 移行が冪等で全募集回・Plan・馬選択を保持し、各募集回が有効なPlanを最低1件持つ

### Issue D: 保存・適用責任の分離

- 複数募集回UI導入前に、対象募集回・対象Planのみをupsertする保存基盤を完成させる
- 非アクティブな募集回とPlanを保持する
- 募集回設定のcapture/applyとPlan状態のcapture/applyを先に分離する
- `getPlanData()` の全体置換を廃止し対象エンティティをマージする
- 完了条件: 設定・filter・馬選択が承認済み所有者へだけ保存され、現在募集回・現在Plan以外のデータが保持される
...
依存順序は `A -> D -> B -> C -> E` とする。後続Issue数は **5件** である。Issue Aでv4モデルと移行・検証基盤を用意した後、Issue Dで非破壊upsertとcapture/apply責任の分離を完成させる。その安全な保存基盤の上にIssue Bの募集回UI、Issue Cの募集回配下Plan管理を順に導入し、最後にIssue Eで統合検証する。
...
- [x] 各Recruitment Roundが最低1件のPlanを持つdefault Plan生成規則と、`activePlanId` の参照整合性検証を定義した。
- [x] 後続Issueを5件へ再編した。
- [x] 非破壊な保存基盤を複数募集回UIより先に完成させる依存順序 `A -> D -> B -> C -> E` を定義した。
...
|---|---|---|---|
| 1.1.1 | 2026-07-15 | Approved | ArchitectureレビューのBlockerを反映。保存・適用責任の分離をUI導入より先行する依存順序へ変更し、Plan 0件時のdefault Plan生成、activePlanId整合性、書込み前検証、冪等性を明文化。 |
