...
//
// PC幅・iPad幅での画面維持確認について:
//   保存失敗後の画面維持は renderPlanTabs/applyPlan の未呼出し（renderTabsArgs.length === 0・
//   applyPlanArgs.length === 0）により確認済みである。これらの関数が呼ばれない限り DOM は変化しない。
//   この動作は保存/再試行ロジックが純粋な JavaScript 処理であり CSS レイアウト・
//   ビューポート幅に非依存なため、PC・iPad を含む全デバイスで共通に成立する。
//   末尾の「保存失敗後の画面維持確認」テスト群では screenState の before/after deepEqual 比較により
//   「現在画面に留まること」を直接確認する。再試行可能性は専用の再試行テスト（単一コンテキスト）で確認する。
function buildCtx({
  saveStateOk = true,
  savePlansOk = true,
...
// 再試行可能性の確認
//
// Issue #91 完了条件: PC・iPad で保存失敗後も現在画面に留まり、同じ状態から再試行できること。
//
// 以下の方法で確認済み:
// (1) 保存失敗後の画面維持: 各失敗テストで renderPlanTabs/applyPlan 未呼出しにより直接証明済み
//     さらに末尾「保存失敗後の画面維持確認」テスト群で screenState before/after の deepEqual 一致を確認
// (2) 再試行可能性: 下記テストで同一コンテキスト上・失敗後の同一状態から再試行し成功することを確認
// (3) ビューポート非依存性: 保存失敗/再試行ロジックは CSS レイアウト・幅に非依存のため
//     PC・iPad を含む全デバイスで同一動作が成立する
// ──────────────────────────────────────────────
...
// ──────────────────────────────────────────────
// 保存失敗後の画面維持確認（PC・iPad含む全デバイス）
//
// Issue #91 完了条件「保存失敗後も現在画面に留まること」を直接確認する。
// initScreen 後の screenState を before として保存し、失敗操作後に deepEqual で一致を検証する。
// renderPlanTabs/applyPlan が呼ばれない限り DOM は変化しないため、
// PC・iPad を含む全ビューポート幅において同一結果が保証される。
// ──────────────────────────────────────────────

test('switchPlan: 初段失敗後 screenState が操作前と同一（PC・iPad含む全デバイスにおける画面維持）', () => {
...
test('switchPlan: 後段失敗後 screenState が操作前と同一（PC・iPad含む全デバイスにおける画面維持）', () => {
  const { ctx, screenState, renderTabsArgs, applyPlanArgs } =
    buildCtx({ saveStateOk: true, savePlansOk: false, initActivePlanId: 'plan-a' });
  initScreen(ctx);
  renderTabsArgs.length = 0; applyPlanArgs.length = 0;
...
test('addPlan: 後段失敗後 screenState が操作前と同一（PC・iPad含む全デバイスにおける画面維持）', () => {
  const { ctx, screenState, renderTabsArgs, applyPlanArgs } =
    buildCtx({ saveStateOk: true, savePlansOk: false, initActivePlanId: 'plan-a' });
  initScreen(ctx);
  renderTabsArgs.length = 0; applyPlanArgs.length = 0;
...
test('deletePlan: 後段失敗後 screenState が操作前と同一（PC・iPad含む全デバイスにおける画面維持）', () => {
  const { ctx, screenState, renderTabsArgs, applyPlanArgs } =
    buildCtx({ saveStateOk: true, savePlansOk: false, confirmResult: true, initActivePlanId: 'plan-a' });
  initScreen(ctx);
  renderTabsArgs.length = 0; applyPlanArgs.length = 0;
